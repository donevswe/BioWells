...
class EditorApp:
    def __init__( self, master, dataframe , edit_rows = [] ):
        ...
        self.root = master
        self.root.minsize(width=900, height=200)
        self.root.title('database editor')

        self.main = tk.Frame( self.root )
        self.main.pack(fill=tk.BOTH, expand=True)

        self.lab_opt = { 'background':'light sea green', 'foreground':'antique white'  }
...
#       subset the data and convert to giant list of strings (rows) for viewing        
        ...
        #print('that:',self.sub_data)
        self.sub_datstring = self.sub_data.to_string(index=True, col_space=10).split('\n')
...
# MAIN LISTBOX #  th
################
    def _init_lb( self):  # the place where you seecolumn number
...
################
    ...
    def _swap_mode(self,mode='findrep'):
    ...
    def _swap_lab( self,mode='findrep' ):
...
#   make a test dataframe here of integers, can be anything really
    #df = pandas.DataFrame(pandas.np.random.randint(0,100, (10, 8)), columns=['col_%d'%x for x in range( 8 ) ] ) 
    file= '/home/ev/Documents/4thBioinformatics/ApplicationsProgrammering/Evgeniy_Donev_a_Present/output3.xlsx'
    data=pandas.ExcelFile(file)
    data = data.parse("Sheet1", header=None)
    print(data)
    df=pandas.DataFrame(np.array(data), columns=['col_%d'%x for x in range(len(np.array(data)[0])) ] )
    print(df)
#   start
    root      = tk.Tk()
